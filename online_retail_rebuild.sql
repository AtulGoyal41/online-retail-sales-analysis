...
-- ---------------------------------------------------------------------
-- Supporting check: Registered-customer cancelled value
-- Verifies the claim in Finding 4 (Guest vs Registered) that Registered
-- customers' cancellations total roughly -£483,794.70, ~77% of all
-- cancelled value (-£629,855.37). Uses the same filter scope as the
-- CancelledValue measure (IsCancelled only, no base filters), so this
-- is a like-for-like slice of that total, not a mismatched comparison.
-- Independently verified in Power BI; both agree exactly.
-- Verified result: -£483,794.70
-- ---------------------------------------------------------------------
SELECT SUM(Revenue) AS CancelledValue_Registered
FROM online_retail
WHERE IsCancelled = 'Cancelled' AND CustomerType = 'Registered';
